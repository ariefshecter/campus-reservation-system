...
func Insert(db *sql.DB, f Facility, userID string) error {
	...
	`, f.Name, f.Description, f.Location, f.Capacity, f.Price, f.PhotoURL, userID)
	return err
}
...
func FindAll(db *sql.DB) ([]Facility, error) {
	// Kita melakukan LEFT JOIN ke tabel users dua kali:
	// 1. u_cre -> untuk mengambil nama pembuat (created_by)
	// 2. u_upd -> untuk mengambil nama pengupdate (updated_by)
	rows, err := db.Query(`
		SELECT 
			f.id, f.name, COALESCE(f.description, ''), COALESCE(f.location, ''), 
			f.capacity, COALESCE(f.price, 0), COALESCE(f.photo_url, ''), f.is_active,
			COALESCE(u_cre.name, '-'), 
			COALESCE(u_upd.name, '')
		FROM facilities f
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&f.Capacity, &f.Price, &f.PhotoURL, &f.IsActive,
			&f.CreatedByName, &f.UpdatedByName, // Scan field baru
		); err != nil {
			return nil, err
		}
...
func FindByID(db *sql.DB, id string) (Facility, error) {
	...
	err := db.QueryRow(`
		SELECT id, name, COALESCE(description, ''), COALESCE(location, ''), capacity, COALESCE(price, 0), COALESCE(photo_url, ''), is_active
		FROM facilities WHERE id = $1 AND deleted_at IS NULL
	`, id).Scan(&f.ID, &f.Name, &f.Description, &f.Location, &f.Capacity, &f.Price, &f.PhotoURL, &f.IsActive)
	return f, err
}
...
func Update(db *sql.DB, id string, f Facility, userID string) error {
	...
	`, f.Name, f.Description, f.Location, f.Capacity, f.Price, f.PhotoURL, userID, id)
	return err
}
